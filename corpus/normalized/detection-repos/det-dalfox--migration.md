...
| :--- | :--- | :--- |
| `--concurrence ` | `--workers ` | Modernized naming. Dictates the number of concurrent scanning workers. |
| `-C, --cookie ` | `--cookies ` | Pluralized for consistency. Multiple occurrences are supported. |
| `-p ` | `-p, --param ` | Enhanced parameter type scoping (e.g. `-p id:query`, `-p sort:body`). |
...
| :--- | :--- | :--- |
| `--use-bav``--skip-bav` | None. | **Deprecated**. Basic Another Vulnerability (BAV) checks are removed to strictly focus v3 as a specialized XSS scanner. Use dedicated vulnerability scanners for non-XSS checks. |
| `--found-action ``--found-action-shell` | Use REST API **webhooks** or CLI stdout **piping** (e.g., `dalfox scan ... \| post-script.sh`). | **Security Risk**. Arbitrary shell execution flags were removed to mitigate Remote Code Execution (RCE) risks and concurrency bottlenecks. |
| `--skip-headless``--force-headless-verification` | Automatically handled via static analysis. | **Engine Replaced**. The slow and resource-heavy Headless Chrome browser (`chromedp`) was completely removed. Dalfox v3 implements an extremely fast, compiler-grade JavaScript static parser (`oxc`) to analyze data-flows and trace DOM-XSS. |
| `--grep ``--skip-grep` | None. | **Engine Replaced**. Simple regex-based response matching is replaced by context-aware JS AST analysis for maximum accuracy. |
| `--report``--report-format` | Use `-f markdown -o ` or `-f sarif -o `. | **Standardization**. Custom report generation flags were consolidated into unified output format parameters. |
...
* **Hard Time Budget (`--scan-timeout `)**: Bounds the total scan time per target to prevent scanning loops on partially-hung remote servers.
* **Payload Cap (`--max-payloads-per-param `)**: Restricts the maximum number of tested payloads per parameter to prevent massive request bursts during combinatorial tests (bypasses + encoders).
