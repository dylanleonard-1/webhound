...
You can also specify it using the [`http-equiv`](/en-US/docs/Web/HTML/Reference/Elements/meta/http-equiv) attribute of your document's meta element, and this is a useful option for some use cases, such as a client-side-rendered SPA which has only static resources, because you can then avoid relying on any server infrastructure. However, this option does not support all CSP features.
...
- uses the same nonce for all `` tags in the document
...
1. calculates a hash of the script contents using a hash function (one of SHA-256, SHA-384, or SHA-512)
...
- The Function/Function() constructor:

  ```js
  const sum = new Function("a", "b", "return a + b");
  ```

- The string argument to Window.setTimeout() and Window.setInterval():
...
- block all uses of the `` element to set a base URI.
...
We've seen above that inline JavaScript is disallowed by default in a CSP. With nonces or hashes, a developer can use inline `` tags, but you'll still need to refactor code to remove other disallowed patterns, including inline event handlers, `javascript:` URLs, and uses of `eval()`. For example, inline event handlers should usually be replaced with calls to EventTarget.addEventListener():
...
The [`frame-ancestors`](/en-US/docs/Web/HTTP/Reference/Headers/Content-Security-Policy/frame-ancestors) directive can be used to control which documents, if any, are allowed to embed this document in a nested browsing context such as an iframe. This is an effective protection against clickjacking attacks, because these attacks depend on embedding the target site in a site controlled by the attacker.
...
The [`require-trusted-types-for`](/en-US/docs/Web/HTTP/Reference/Headers/Content-Security-Policy/require-trusted-types-for) and [`trusted-types`](/en-US/docs/Web/HTTP/Reference/Headers/Content-Security-Policy/trusted-types) directives can be used to enforce the [Trusted Types API](/en-US/docs/Web/API/Trusted_Types_API).
This allows you to defend against client-side [cross-site scripting (XSS)](/en-US/docs/Web/Security/Attacks/XSS) attacks by mandating that any input must be passed through a transformation function, providing an opportunity to make it safe before it is sent to a web platform API that might otherwise execute it as code.

### Injection sinks and sanitization

Some APIs in the web platform are known as _injection sinks_. These are APIs that can be passed some input, usually in the form of a string, and that can interpret that input as code. In this guide we've already seen `eval()`, but there are many other injection sinks, such as Element.innerHTML or Document.write().
...
```js example-bad
const possiblyXSS = "I might be XSS";
...
Note that unlike a normal content security policy, a report-only policy cannot be delivered in a `` element.

### Violation reporting

The recommended method for reporting CSP violations is to use the [Reporting API](/en-US/docs/Web/API/Reporting_API), declaring endpoints in Reporting-Endpoints and specifying one of them as the CSP reporting target using the `Content-Security-Policy` header's report-to directive.
...
The report is a serialized form of the CSPViolationReport object containing a `type` property with a value of `"csp-violation"`.
