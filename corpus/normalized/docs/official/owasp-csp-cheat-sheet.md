...
- `style-src` controls from where styles get applied to a document. This includes `` elements, `@import` rules, and requests originating from a `Link` HTTP response header field.
...
- `base-uri` specifies the possible URLs that the `` element can use.
- `plugin-types` limits the types of resources that can be loaded into the document (*e.g.* `application/pdf`). 3 rules apply to the affected elements, `` and ``:
...
- `frame-ancestors` restricts the URLs that can embed the requested resource inside of  ``, ``, ``, ``, or `` elements.
    - If this directive is specified in a `` tag, the directive is ignored.
