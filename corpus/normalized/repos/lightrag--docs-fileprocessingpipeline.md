...
For image build details, GPU driver setup, model weight locations, etc., refer to the official README: .
...
| `DOCLING_MAX_POLLS` | `240` | Maximum poll iterations; raises `TimeoutError` when exceeded;default wait time ≈ 5 × 240 (about 20 minutes) |
...
| `CHUNK_V_SENTENCE_SPLIT_REGEX` | `(? `DEFAULT_CHUNK_P_SIZE` (2000), **skipping** ③ legacy constructor field `LightRAG(chunk_token_size=…)` and ④ legacy env `CHUNK_SIZE`. See the `CHUNK_P_SIZE` row in §3.2 for the rationale.
...
### 4.3 MinerU Raw Artifacts Directory `.mineru_raw/`

The `mineru` engine writes the complete artifacts returned by the MinerU service (`content_list.json` + optional `full.md` / `middle.json` / `layout.pdf` / `images/`, etc.) into the `__parsed__/.mineru_raw/` directory during parsing, and writes `_manifest.json` as the integrity validation file.
...
### 4.4 Docling Raw Artifacts Directory `.docling_raw/`
...
```text
__parsed__/.docling_raw/
├── _manifest.json
├── .json        # DoclingDocument JSON (contains pages[].image base64)
├── .md          # Markdown form, for human inspection
└── artifacts/
...
PENDING ─►├─ parse_queues["mineru"]  ─► [mineru pool  × N2] ─┼─► q_analyze ─►[analyzer × N4] ─► q_process ─►[processor × N5]
          ...
          └─ parse_queues[] ─► [custom pool] ┘   ← created per ParserSpec.queue_group
```
