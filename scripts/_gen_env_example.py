...
# ---------------------------------------------------------------------------
# Secret storage / encryption at rest  [prod-required for provider OAuth] [secret]
# ---------------------------------------------------------------------------
# Phase 4.1 encrypted-secret store. Provider OAuth tokens (Cloudflare, etc.) are
# encrypted with these keys before they touch the database. Format is one or more
# versioned Fernet keys:  <version>:<fernet-key>[,<version>:<fernet-key>...]
# Generate a key: python3 -c "from cryptography.fernet import Fernet; print('v1:'+Fernet.generate_key().decode())"
# FAIL-CLOSED: if unset, provider OAuth connect/callback REFUSES to store any token
#   (the Cloudflare flow returns 503 / encryption_not_configured). In dev with this
#   left blank, an EPHEMERAL key is used and stored secrets do NOT survive restart.
ENCRYPTION_KEYS=                         # [secret] e.g. v1:xxxxx=
# Which key version above is the ACTIVE one for new writes (defaults to the first).
...
#   * Create the OAuth client + complete publisher verification in Cloudflare.
#   * Register this EXACT callback URL on the Cloudflare OAuth client:
#       {API_BASE_URL}/integrations/cloudflare/callback
